...
class Pasien_model extends CI_Model {
    ...
    // 1. Ambil data profil pasien berdasarkan id_user dari session
    public function get_profil_pasien($id_user) {
        return $this->db->get_where('pasien', array('id_user' => $id_user))->row();
    }

    // 2. Ambil kunjungan aktif pasien yang berstatus 'menunggu'
    public function get_kunjungan_aktif($id_pasien) {
        $this->db->where('id_pasien', $id_pasien);
        $this->db->where('status_konsultasi', 'menunggu');
        return $this->db->get('konsultasi')->row();
    }

    // 3. Hitung nomor urut antrean berdasarkan ID pendaftaran yang lebih kecil atau sama
    public function hitung_nomor_urut($id_dokter, $id_konsultasi) {
    ...
    // 4. Ambil informasi data dokter berdasarkan ID dokter
    public function get_dokter_by_id($id_dokter) {
        return $this->db->get_where('dokter', array('id_dokter' => $id_dokter))->row();
    }

    // 5. Fitur Pencarian Dokter dan Jadwal Praktik
    public function cari_dokter_dan_jadwal($keyword = null) {
    ...
    // 6. Ambil Riwayat Rekam Medis milik pasien tertentu
    public function get_riwayat_rekam_medis($id_pasien) {
